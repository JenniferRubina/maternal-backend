...
router.post('/appointment', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT a.visit_type, a.appointment_datetime, h.name AS doctor_name , a.id, a.status FROM appointment a JOIN healthcare_worker h ON a.doctor = h.id WHERE a.rch_id = $1` , [rch_id]  
    );
    if (result.rows.length > 0) {
      res.status(200).json(result.rows);
    } else {
